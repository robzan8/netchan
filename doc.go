...
receiver's buffer. Each time the sender issues an item, it decrements the credit by one.
If the credit becomes zero, the sender must suspend sending items, as the receive buffer
could be full. On the receive side, when a good number of items have been taken out of
the channel (and the buffer has new free space), the number is communicated back to the
sender with an increase credit message.

The receive channel's capacity can affect performance: a small buffer could cause the
